...
**القسمان الجدد في لوحة المطور (سبتمبر 2026):**
- **«غرفة الوكيل 🤖»** — تجهيز احترافي: فحص حي لـ 5 شروط (OpenRouter/GitHub PAT/طلب/سعر/مستودع) + شاشة أدوات بـ 11 مفتاح + حد الجولات + وضع المحاكاة + إصلاح تلقائي
- **«الموصلات 🔌»** — كل التكاملات في شاشة واحدة بفحص حي حقيقي (GitHub/Supabase/Firebase/OpenRouter) + دليل توزيع Firebase اليدوي
...
3. تصحيح خطأ إملائي `فشل جبل` → `فشل جبل`
...
### إصلاحات تمت (سبتمبر 2026 — commits c6183eb و e7bb522)
- أخطاء بناء أُصلحت: AiTools.kt lambda { } → { _ -> }، BuildCenterSection.kt إزالة val task المزدوج، AgentRoomSection.kt إزالة enabledCodeTools غير المستخدمة
- «غرفة الوكيل 🤖» — شاشة تجهيز احترافي مع فحص حي + شاشة أدوات (11 مفتاح، حد الجولات، وضع المحاكاة، إصلاح تلقائي)
- «الموصلات 🔌» — كل التكاملات في شاشة واحدة بفحص حي حقيقي (GitHub يجلب اسم المستخدم بـ PAT، Supabase، Firebase، OpenRouter) + دليل توزيع Firebase اليدوي الصادق
- إصلاح حرج: فتح المحادثة كان يعرض شاشة فارغة — selectedRequestId لم يُمرر أبداً → أُصلح في AppNavigation.kt
- إصلاح فحص مزدوج: hasRepo في SmartActionBar كان يقرأ من ملف prefs لا يُكتب فيه → وُحّد على ملف الطلبات الحقيقي
- MainActivity: إصلاح صيغة string interpolation ($$price → ${price})
- RealServices: chatWithAssistant تقبل preferredOpenRouterModel
- OpenRouterService: chatWithHistory و chatWithTools لمحادثة متعددة الأدوار
- GitHubRepoClient: getAuthUser() للتحقق من صلاحية PAT
- البناء لم يُتحقق محلياً (لا JDK/SDK في هذه البيئة) — يُبنى عبر Actions
